...
sequences = full_df["sequence"].tolist()
labels = full_df["sense"].tolist()

leng = [len(s) for s in sequences]
print("Max seq length:", max(leng))
...
# defing a tokenizer function using the "tokenizer" obj. that can trim/pad sequences as required & tokenize
...
# Initializing backbone + decoder layers for binary classification & setting other parameters
n_classes = 2
...
# unfreezing the backbone layer for full hypertuning
backbone.train()

# getting the backbone hidden size to pass as a parameter to decoder layer 
d_model = backbone.model.d_model if hasattr(backbone, "model") else 256

# instantiating the decoder layer & passing params
decoder = SequenceDecoder(d_model=d_model, d_output=n_classes, mode="last").to(device)

# defining the loss function & optimizer for both backbone & decoder layers
...
# training the backbone + decoder layers & predicting classes of test sequences over 10-folds
kf = KFold(n_splits=10, shuffle=True, random_state=42)
f1_scores = []

# splitting data into train & test sets
for fold, (train_idx, test_idx) in enumerate(kf.split(sequences, labels), 1):
    print(f"\n----- Fold {fold} -----")
    set_seed(42)
    ...
    # getting max. sequence lengths in train & test data to pass as a param to tokenizer
    max_train_len = max(len(s) for s in x_train)
    max_test_len  = max(len(s) for s in x_test)

    # tokenizing the sequences
    x_train_tensor = tokenize_sequences(x_train, max_train_len)
    x_test_tensor  = tokenize_sequences(x_test, max_test_len)
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in train_loader:
            hidden_states = backbone(batch_x)       
            logits = decoder(hidden_states)
            # using the last token's logits (per sequence) for classification 
            logits = logits[:, -1, :]               
    ...
    # classifying the test sequences in batches & calculating f1-score
    backbone.eval()
    decoder.eval()
    ...
    print(f"Actual labels : {all_labels}")
    f1 = f1_score(all_labels, all_preds, average="weighted")
    f1_scores.append(f1)
    print(f"Fold {fold} f1 score : {f1:.4f}")

# printing mean f1-score & sd across 10 folds
print("\nf1 scores across folds :", f1_scores)
print(f"Mean f1 : {np.mean(f1_scores):.4f}, sd: {np.std(f1_scores):.4f}")
