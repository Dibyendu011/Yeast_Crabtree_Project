...
embed_tensor = [[] for _ in range(len(full_df))] # stores vectors for all tokens without pooling
i = 0
for seq in full_df["sequence"]:
...
torch.save(embed_tensor, "tensors_base_1.pt")
...
# embed_tensor = torch.load("tensors_base_1.pt", map_location=device)
...
count_df.to_csv('count_matrix_base.csv', index=False)
