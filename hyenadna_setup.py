...
# applying on a random test sequence to check if setup was successfull
...
def inference_single():
    ...
    model = HyenaDNAPreTrainedModel.from_pretrained('./checkpoints',
    ...
    tokenizer = CharacterTokenizer(characters=['A', 'C', 'G', 'T', 'N'],
...
# estimating maximum k-mer length for our model
sequences = []
for i in range (64) :
  sequences.append(full_df["sequence"][i])
leng = []
for i in range (len(sequences)):
  leng.append(len(sequences[i]))
print(f"Longest sequence length : {max(leng)}")


# selecting a model & getting word embedded values for all sequences in "full_df" table
...
tokenizer = CharacterTokenizer(characters=['A', 'C', 'G', 'T'],
